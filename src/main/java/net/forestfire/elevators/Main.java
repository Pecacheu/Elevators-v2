...
public boolean onCommand(CommandSender s, Command c, String l, String[] a) {
	if(c.getName().equalsIgnoreCase("elev")) {
		if(a.length == 1 && a[0].equals("list")) {
			...
			s.sendMessage(el.size()+" Elevators:"); for(Elevator e: el) {
				Floor f=e.floor; Block b=e.sGroups.get(0).get(0); s.sendMessage("Elevator in §d"+f.world.getName()
					+" §rat §b["+b.getX()+","+b.getZ()+"]§r; Size: §b"+(f.xMax-f.xMin)+"x"+(f.zMax-f.zMin));
			}
...
public void onSignChange(SignChangeEvent e) { Block sign = e.getBlock();
	...
	if(e.getLine(0).equalsIgnoreCase("[call]") && Conf.hasPerm(e.getPlayer(), PERM_CREATE, true)) { //Call Sign:
		e.line(0,Conf.sc("> ERROR <")); e.line(1,Conf.sc("")); e.line(2,Conf.sc(""));

		//Is sign in elevator?
		CSData ret = Elevator.fromCallSign(sign); if(ret==null) {
			e.line(0,Conf.sc(Conf.ERROR)); Conf.err("onSignChange:CallSign", "No elevator found!"); return;
		}
	...
	} else if(e.getLine(0).equalsIgnoreCase("[elevator]") && Conf.hasPerm(e.getPlayer(), PERM_CREATE, true)) { //Elevator Sign:
		...
		if(elev!=null) { //Existing Elevator:
			...
			for(int i=0,l=elev.sGroups.length; i<l; i++) { //Check for col XZ match:
				Block ds=elev.sGroups.get(i).get(0); if(ds.getX()==sX && ds.getZ()==sZ) { col=i; break; }
			}
			if(col != -1) { //Existing Column:
				...
				for(int k=0,m=elev.sGroups.length; k<m; k++) if(k != col) { //Rebuild other cols to match
					Block s=elev.sGroups.get(k).get(0), b=w.getBlockAt(s.getX(), sY, s.getZ());
					b.setType(s.getType()); b.setBlockData(s.getBlockData()); Conf.addSignBlock(b); Conf.line(b,0,Conf.TITLE);
					elev.sGroups.set(k,Elevator.rebuildSignList(new Location(w,s.getX(),0,s.getZ())));
				}
			} else { //New Column:
				WallSign d=(WallSign)sign.getState().getBlockData(); Material t=sign.getType(); sign.setType(Conf.AIR);
				ChuList<Block> sRef=elev.sGroups.get(0); int sl=sRef.length; sList=new ChuList<>(sl);
			...
			Floor f=Floor.getFloor(sign, null); if(f==null) {
				e.line(0,Conf.sc(Conf.ERROR)); Conf.err("onSignChange:ElevSign:NewElev", "Floor not found!"); return;
			}
		...
		if(elev.noDoor) elev.sGroups.get(0).forEach((s) -> Conf.line(s,2,"")); //NoDoor
		...
		if(elev.noDoor || sl.equalsIgnoreCase("[nodoor]")) { //Enable NoDoor Mode:
			elev.noDoor = true; Block f=elev.sGroups.get(0).get(0);
		...
		for(int i=0,l=sList.length; i<l; i++) {
			elev.setDoors(i, i==0); elev.floor.addFloor(sList.get(i).getY()-2, false, true);
		}
...
private void onDestroyCallSign(Cancellable e, Player p, Block b) {
	...
	if(!Conf.hasPerm(p, PERM_CREATE, true)) { Conf.err("onBlockBreak:CallSign", "No permission!"); e.setCancelled(true); return; }
...
private void onDestroyElevSign(Cancellable e, Player p, Block b) {
	...
	if(!Conf.hasPerm(p, PERM_CREATE, true)) { e.setCancelled(true); Conf.err("onBlockBreak:ElevSign", "No permission!"); return; }
...
public void onPlayerInteract(PlayerInteractEvent e) { synchronized(Conf.API_SYNC) { ConfData ref = new ConfData();
	Action act = e.getAction(); if(act == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock().getBlockData() instanceof WallSign) {
		if(Conf.isElevSign(e.getClickedBlock(), ref, e.getPlayer()) && !((Elevator)ref.data).floor.moving) { //Select Floor:
			...
			e.setCancelled(true);
		} else if(Conf.isCallSign(e.getClickedBlock(), ref, e.getPlayer()) && !((CSData)ref.data).elev.floor.moving) { //Call Sign Click:
			Elevator elev=((CSData)ref.data).elev; int from=elev.getFloor(), to=((CSData)ref.data).index;
			LAST_ELEV=elev;

			//Call Elevator to Floor:
			if(from != to) {
				e.getPlayer().sendMessage(Conf.MSG_CALL); elev.gotoFloor(from, to, false);
			} else elev.doorTimer(to); //Re-open doors if already on level.

			e.setCancelled(true);
		}} else if((act == Action.RIGHT_CLICK_AIR || act == Action.RIGHT_CLICK_BLOCK && (e.getItem()==null || e.getPlayer().isSneaking()))
