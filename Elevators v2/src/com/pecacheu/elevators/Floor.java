...
public class Floor {
    ...
    public static Floor getFloor(Block b, Elevator parent) {
        ...
        int bX = b.getX(), h = b.getY() - 2, bZ = b.getZ();
        Material fType = world.getBlockAt(bX, h, bZ).getType();
        if (Conf.BLOCKS.indexOf(fType.toString()) == -1 || world.getBlockAt(bX, h + 1, bZ).getType() != Conf.AIR) {
        ...
        if (face != "EAST") while (xN <= Conf.RADIUS_MAX) {
            if (world.getBlockAt(bX - xN, h, bZ).getType() != fType) break;
            xN++;
        }
        if (face != "NORTH") while (zP <= Conf.RADIUS_MAX) {
            if (world.getBlockAt(bX, h, bZ + zP).getType() != fType) break;
            zP++;
        }
        if (face != "SOUTH") while (zN <= Conf.RADIUS_MAX) {
            if (world.getBlockAt(bX, h, bZ - zN).getType() != fType) break;
            zN++;
        }
    ...
    public int addFloor(double h, boolean isMoving, boolean dontDelete, Integer forceID) {
        if (!dontDelete) {
            removeFallingBlocks();
            elev.resetElevator(true);
        }
        if (isMoving) { //Create FallingBlock Floor:
            ChuList<FallingBlock> blocks = new ChuList<FallingBlock>();
            moving = true;
            ...
            int ind;
            if (forceID != null) ind = forceID;
            else ind = Conf.findFirstEmpty(Conf.movingFloors);
            Conf.movingFloors.set(ind, blocks);
            return ind;
        } else { //Create Solid Floor:
            for (int x = xMin; x <= xMax; x++)
                for (int z = zMin; z <= zMax; z++) world.getBlockAt(x, (int) h, z).setType(fType);
        }
        return 0;
    }
    ...
    public int addFloor(double h, boolean isMoving) {
        return addFloor(h, isMoving, false, null);
    }
    ...
    public void removeFallingBlocks() {
        ...
        for (int i = 0, l = el.length; i < l; i++) {
            ...
            if ((loc.getX() >= xMin - 0.5 && loc.getX() <= xMax + 0.5) && (loc.getY() >= yMin - 0.5 && loc.getY
                    () <= yMax + 0.5) && (loc.getZ() >= zMin - 0.5 && loc.getZ() <= zMax + 0.5))
                ((Entity) el[i]).remove();
        }
    }
}