...
public class Conf {
    ...
    public static String TITLE, CALL, ERROR, L_ST, L_END, NODOOR, MSG_GOTO_ST, MSG_GOTO_END, MSG_CALL, NOMV, M_ATLV, ATLV,
            C_UP, UP, C_DOWN, DOWN;
    public static int RADIUS_MAX, MOVE_RES, DOOR_HOLD, SAVE_INT;
    ...
    public static boolean saveConfig() {
        ...
        if (!path.exists()) data = newConfig(path);
        else {
            try { //Read Current Config File:
                ...
                while (p < 3000) {
                    int read = file.read();
                    if (read < 0 || read >= 65535) break;
                    data += fromCharCode(read);
                    p++;
                }
                file.close();
            } catch (IOException e) {
        ...
        int bPos = data.lastIndexOf("blockList:");
        ...
        int sPos = data.lastIndexOf("elevators:");
        ...
        Object[] eKeys = elevators.keySet().toArray();

        //Generate Compressed Elevator Data:
        for (int i = 0, l = eKeys.length; i < l; i++)
            eList.set((String) eKeys[i], elevators.get(eKeys[i]).toSaveData());

        //Append New Data And Save File:
        String eStr = eConf.saveToString();
        eStr.substring(0, eStr.length() - 1);
        data += eStr;
        Writer file;
        try {
    ...
    public static Object loadConfig() {
        try {
            ...
            boolean pathFound = path.exists();
            if (pathFound) config = YamlConfiguration.loadConfiguration(path);
            else config = new YamlConfiguration();

            config.setDefaults(defaults);
            movingFloors = new ChuList<ChuList<FallingBlock>>();
            CLTMR = null;
            ...
            BLOCKS = new ChuList<String>();
            BL_SPEED = new ChuList<Integer>();
            if (bList != null) {
                ...
                for (int b = 0, g = bKeys.length; b < g; b++) { //Remove any items from block list that aren't solid blocks:
                    ...
                    if (mat != null && mat.isSolid()) {
                        BLOCKS.push((String) bKeys[b]);
                        BL_SPEED.push(bList.getInt((String) bKeys[b]));
                    }
                }
            }
            ...
            Integer eCnt = 0;
            if (eList != null) {
                Object[] eKeys = eList.getKeys(false).toArray();
                for (int i = 0, l = eKeys.length; i < l; i++) {
                    Elevator elev = Elevator.fromSaveData(eList.getStringList((String) eKeys[i]));
                    if (elev != null) elevators.put((String) eKeys[i], elev);
                    else {
                        eCnt++;
                        err("loadConfig", "fromSaveData returned null at ID " + eKeys[i]);
                    }
                }
            }
    ...
    public static void doConfigLoad() {
        doConfigLoad(null);
    }
    ...
    public static Location locFromString(String str) {
        ...
        if (world == null) {
            err("locFromString", "World '" + data[0] + "' not found!");
            return null;
        }
    ...
    public static void setPowered(Block block, boolean onOff) {
        switch (block.getType()) {
            case REDSTONE_WIRE:
                ...
                break;
            case LEVER:
                BlockState stL = block.getState();
                ((org.bukkit.material
                        .Lever) stL.getData()).setPowered(onOff);
                stL.update();
                break;
    ...
    public static void setSign(Block sign, String[] lines) {
        ...
        state.setLine(0, lines[0] == null ? TITLE : lines[0]);
        state.setLine(1, lines[1] == null ? "" : lines[1]);
        state.setLine(2, lines[2] == null ? "" : lines[2]);
        state.setLine(3, lines[3] == null ? "" : lines[3]);
        state.update();
    }

    public static void setSign(Block sign, String lineOne) {
        org.bukkit.block.Sign state = ((org.bukkit.block.Sign) sign.getState());
        state.setLine(0, lineOne == null ? TITLE : lineOne);
        state.setLine(1, "");
        state.setLine(2, "");
        state.setLine(3, "");
        state.update();
    ...
    public static void addSignBlock(Block s, byte d) {
        Block b = Conf.getSignBlock(s, d);
        if (!b.getType().isSolid()) b.setType(Conf.DOOR_SET);
    }
    ...
    public static boolean isElevSign(Block sign, ConfData ref, Player player, String perm) {
        if (!hasPerm(player, perm)) return false;
        if (sign.getType() == Material.OAK_WALL_SIGN && TITLE.equals
                (lines(sign)[0])) {
            ref.data = Elevator.fromElevSign(sign);
    ...
    public static boolean isCallSign(Block sign, ConfData ref, Player player, String perm) {
        if (!hasPerm(player, perm)) return false;
        if (sign.getType() == Material.OAK_WALL_SIGN && CALL.equals
                (lines(sign)[0])) {
            ref.data = Elevator.fromCallSign(sign);
            return (ref.data != null);
        }
        return false;
    }
    ...
    public static boolean isElevPlayer(Player player, ConfData ref, String perm) {
        if (!hasPerm(player, perm)) return false;
        ref.data = Elevator.fromPlayer(player);
        return (ref.data != null);
    }
    ...
    public static int findFirstEmpty(ChuList<ChuList<FallingBlock>> list) {
        int l = list.length;
        for (int i = 0; i < l; i++) if (list.get(i) == null) return i;
        return l;
    }
    ...
    public static String unpackFile(String internalPath, File dest) {
        ...
        try {
            while (p < 3000) {
                ...
                if (read < 0 || read >=
                        65535) break;
                str += fromCharCode(read);
                p++;
            }
            stream.close();
            Writer file = new BufferedWriter(new OutputStreamWriter(new
                    FileOutputStream(dest), "UTF-8"));
            file.write(str);
            file.close(); //<- Needed for Proper UTF-8 Encoding.
        } catch (Exception e) {
    ...
    public static String c(String str) {
        ...
        for (int i = 1, l = clr.length; i < l; i++)
            cStr += org.bukkit.ChatColor
    ...
    public static Block getBlockBelowPlayer(Player p, boolean above) {
        ...
        int pX = (int) Math.floor(loc.getX()), pZ = (int) Math.floor(loc.getZ());
        double pY = loc.getY();
        return p.getWorld().getBlockAt(pX, (int) (above ? Math.ceil(pY + 1.99) : Math.floor(pY - 1)), pZ);
    }

    public static Block getBlockBelowPlayer(Player p) {
        return getBlockBelowPlayer(p, false);
    }
}
