...
public class Main extends JavaPlugin implements Listener {
    ...
    public void onSignChange(SignChangeEvent event) {
        synchronized (Conf.API_SYNC) {
            if (!Conf.DISABLED && event.getBlock().getType() == Material.OAK_WALL_SIGN) {
                if (event.getLine(0).equalsIgnoreCase("[call]") && Conf.hasPerm(event.getPlayer(), PERM_CREATE)) { //Call Sign:
                    ...
                    //Is sign in elevator?
                    CSData ret = Elevator.fromCallSign(block);

                    if (ret == null) {
                        getServer().broadcastMessage("Ret: is null");
                    ...
                    event.setLine(0, Conf.CALL);
                    event.setLine(3, elev.csData.get(csNum));
                    ...
                    //Build sign list:
                    ChuList<Block> sList = Elevator.rebuildSignList(block.getLocation());
                    int ind = 0;
                    ...
                    if (elev != null) { //Existing Elevator:
                        ...
                        for (int i = 0, l = elev.sGroups.length; i < l; i++) {
                            Block dSign = elev.sGroups.get(i)
                                    .get(0);
                            if (dSign.getX() == block.getX() && dSign.getZ() == block.getZ()) {
                                column = i;
                                break;
                            }
                        } //Check if X and Z match existing sList.
                        if (column != -1) { //Existing Column:
                            ind = column;
                            for (int k = 0, m = elev.sGroups.length; k < m; k++)
                                if (k != ind) { //Iterate through sGroups:
                                    ...
                                    for (int i = 0, l = oList.length; i < l; i++)
                                        oList.get(i).setType(Conf.AIR); //Delete old signs in other columns.
                                    oList = new ChuList<Block>();
                                    for (int i = 0, l = sList.length; i < l; i++) { //Rebuild to match new column.
